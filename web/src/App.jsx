  ...
  return (
    <div className="app">
      <header className="header">
        <div className="header-ribbon header-ribbon-universes">
          <div className="header-brand">
            <AstroLogo className="header-logo" />
          </div>
          <div className="universe-ribbon" role="tablist" aria-label="Universes">
            {universes.map((u) => (
              <button
                key={u.id}
                type="button"
                role="tab"
                aria-selected={u.id === currentUniverseId}
                className={`universe-tab ${u.id === currentUniverseId ? 'active' : ''}`}
                onClick={() => switchUniverse(u.id)}
                title={u.name}
              >
                <span className="universe-tab-label">{u.name}</span>
              </button>
            ))}
          </div>
          <button
        ...
        {(pinnedItems.markdowns.length > 0 || pinnedItems.documents.length > 0 || pinnedItems.links?.length > 0 || pinnedItems.diagrams?.length > 0 || pinnedItems.tables?.length > 0 || pinnedItems.scripts?.length > 0) && (
          <div className="header-ribbon header-ribbon-pinned">
            <div className="pinned-ribbon" aria-label="Pinned items">
              {pinnedItems.markdowns.map((n) => (
                <button key={`n-${n.id}`} type="button" className="pinned-tab pinned-markdown" onClick={() => openMarkdownTab(n)} title={n.title || 'Untitled'}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
                    <polyline points="14 2 14 8 20 8" />
                  </svg>
                  <span className="pinned-tab-label">{n.title || 'Untitled'}</span>
                </button>
              ))}
              {pinnedItems.documents.map((d) => (
                <button key={`d-${d.path}`} type="button" className="pinned-tab pinned-doc" onClick={() => setQuickView({ ...d, type: 'doc' })} title={d.name}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z" />
                    <polyline points="13 2 13 9 20 9" />
                  </svg>
                  <span className="pinned-tab-label">{d.name}</span>
                </button>
              ))}
              {(pinnedItems.links || []).map((l) => (
                <button key={`l-${l.id}`} type="button" className="pinned-tab pinned-link" onClick={() => window.open(l.url, '_blank', 'noopener,noreferrer')} title={l.url}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" />
                    <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" />
                  </svg>
                  <span className="pinned-tab-label">{l.title || l.url}</span>
                </button>
              ))}
              {(pinnedItems.diagrams || []).map((d) => (
                <button key={`dg-${d.id}`} type="button" className="pinned-tab pinned-diagram" onClick={() => openDiagramTab(d)} title={d.title || 'Untitled Diagram'}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" /><rect x="14" y="14" width="7" height="7" /><rect x="3" y="14" width="7" height="7" />
                  </svg>
                  <span className="pinned-tab-label">{d.title || 'Untitled'}</span>
                </button>
              ))}
              {(pinnedItems.tables || []).map((t) => (
                <button key={`tb-${t.id}`} type="button" className="pinned-tab pinned-diagram" onClick={() => openTableTab(t)} title={t.title || 'Untitled Table'}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="3" y1="15" x2="21" y2="15"/><line x1="9" y1="3" x2="9" y2="21"/>
                  </svg>
                  <span className="pinned-tab-label">{t.title || 'Untitled'}</span>
                </button>
              ))}
              {(pinnedItems.scripts || []).map((s) => (
                <button key={`sc-${s.id}`} type="button" className="pinned-tab pinned-diagram" onClick={() => openScriptTab(s)} title={s.title || 'Untitled Script'}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <polyline points="16 18 22 12 16 6" /><polyline points="8 6 2 12 8 18" />
                  </svg>
                  <span className="pinned-tab-label">{s.title || 'Untitled'}</span>
                </button>
              ))}
            </div>
          </div>
