...
function TablesPanel({ categories, universeId, universes, onPinChange, onEditTable, refreshKey, onLoaded }) {
  ...
  const startNew = () => {
    onEditTable?.({ _new: true, universeId })
  }

  const remove = useCallback(async (tableId) => {
    if (!confirm('Delete this table and all its data?')) return
    await fetch(`/api/tables/${tableId}`, { method: 'DELETE' })
    fetchTables()
    onPinChange?.()
  }, [fetchTables, onPinChange])

  const togglePin = useCallback(async (table) => {
    await fetch(`/api/tables/${table.id}/pin?pinned=${!table.pinned}`, { method: 'PUT' })
    fetchTables()
    onPinChange?.()
  }, [fetchTables, onPinChange])

  const moveToUniverse = useCallback(async (table, targetUniverseId, categoryId) => {
  ...
  const onCsvSelected = async (e) => {
    ...
    if (data.table_id) {
      fetch(`/api/tables/${data.table_id}`)
        .then(r => r.json())
        .then((t) => onEditTable?.(t))
        .catch(() => {})
    }
  }
  ...
  const renderItem = useCallback(({ node, color, highlighted, dimmed }) => {
    const table = node.item
    let colCount = 0
    try { colCount = JSON.parse(table.columns).length } catch { /* ignore bad columns json */ }
    return (
      <CanvasItemRow
        key={node.id}
        node={node}
        ...
        onOpen={() => onEditTable?.(table)}
        actions={(
          <>
            <ActionIconButton className={table.pinned ? 'pinned' : ''} title={table.pinned ? 'Unpin' : 'Pin'} onClick={() => togglePin(table)}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill={table.pinned ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2">
            ...
            />
            <ActionIconButton title="Delete table" onClick={() => remove(table.id)}>
  ...
  }, [moveToUniverse, onEditTable, remove, togglePin, universeId, universes])
  ...
  return (
    <ContentCanvasShell
      ...
      emptyBody="Click + to create one, or import a CSV."
      toolbar={(
        <>
          <input
            ref={csvInputRef}
          ...
          />
          <button type="button" className="browse-add-button" onClick={() => csvInputRef.current?.click()} title="Import CSV as new table">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="7 10 12 15 17 10" /><line x1="12" y1="15" x2="12" y2="3" />
            </svg>
            Import CSV
          </button>
          <button type="button" className="browse-add-button" onClick={startNew} title="New table">
